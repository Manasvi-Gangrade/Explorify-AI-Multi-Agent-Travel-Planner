...
export function DestinationSearch({
  ...
  const listId = useRef(`dest-list-${Math.random().toString(36).slice(2)}`).current;
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    navigate({ to: "/trips", search: { q: query.trim() || undefined } });
  };
    ...
    >
      <div className="grid gap-2 md:grid-cols-[1.4fr_1fr_0.8fr_auto]">
        <div className="relative">
          ...
          <MapPin className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-azure" />
          <input
            id="dest-input"
            role="combobox"
